...
pub const MAX_KEY_LENGTH: usize = 32; // bytes
pub const MAX_IV_LENGTH: usize = 48; // bytes
pub const MAX_STREAM_LENGTH: usize = 340; // bytes, maximum stream generated by one call
pub const SHIFT_REGISTER_LENGTH: usize = 17; // words
pub const ROUND_OUTPUT_LENGTH: usize = 20; // bytes
...
impl Turing {
    ...
    fn build_s_box_tables(&mut self) {
        for j in 0..256 {
            ...
            for i in 0..self.key_length {
                k = SBOX[(Self::get_byte(self.mixed_key[i], 0) ^ k) as usize] as u32;
        ...
        for j in 0..256 {
            ...
            let mut k = j as u32;
            for i in 0..self.key_length {
                k = SBOX[(Self::get_byte(self.mixed_key[i], 1) ^ k) as usize] as u32;
        ...
        for j in 0..256 {
            ...
            let mut k = j as u32;
            for i in 0..self.key_length {
                k = SBOX[(Self::get_byte(self.mixed_key[i], 2) ^ k) as usize] as u32;
        ...
        for j in 0..256 {
            ...
            let mut k = j as u32;
            for i in 0..self.key_length {
                k = SBOX[(Self::get_byte(self.mixed_key[i], 3) ^ k) as usize] as u32;
    ...
    fn fixed_s(w: u32) -> u32 {
        ...
        w = ((w ^ QBOX[b as usize]) & 0x00ffffff) | ((b as u32) << 24);

        b = SBOX[Self::get_byte(w, 1) as usize];
        w = ((w ^ Self::left_rotate_word(QBOX[b as usize], 8)) & 0xff00ffff)
            | ((b as u32) << 16);

        b = SBOX[Self::get_byte(w, 2) as usize];
        w = ((w ^ Self::left_rotate_word(QBOX[b as usize], 16)) & 0xffff00ff)
            | ((b as u32) << 8);

        b = SBOX[Self::get_byte(w, 3) as usize];
        w = ((w ^ Self::left_rotate_word(QBOX[b as usize], 24)) & 0xffffff00) | b as u32;
